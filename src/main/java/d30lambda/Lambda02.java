...
public class Lambda02 {
    ...
    public static void printCubeOfDistinctOddElements(List<Integer> nums) {
        ...
        nums.
                ...
                filter(t -> t % 2 != 0).//filtrele tekleri yazdır
                map(t -> t * t * t).//küpünü almayı yaptık
                forEach(t -> System.out.print(t + " "));//27 125//sonucu yazdır
    }
    ...
    // u: Akıştan gelenleri tutar
    //t: İlk değeri ve sonra da işlem sonuçlarını tutar


    public static void printSumOfSquaresOfDistinctEvenElements(List<Integer> nums) {
      /*  int toplam = nums.//2,3,3,5,11,10
                stream().//2,3,3,5,11,10
                distinct().//tekrarlı olanlar gitti 2,3,5,11,10
                filter(t -> t % 2 ==0).//çift sayılar 2,10
                map(t -> t * t).//karesini aldı 4 , 100
                reduce(0, (t, u) -> t + u);//akıştan gelenler u ya , ilk değer ve ara işlemler t ye yüklenir t = 4 u = 100 toplam 104*/
    ...
    //reduce'un tek parametreli kullaniminda bir Optional döndürür. Bu, sonucun var olabileceği veya olmayabileceği anlamına gelir.
    //Bu nedenle sonucu almak için get() metodu kullanılır.
    public static void maksimum3(List<Integer> nums) {
        int max = nums.stream().distinct().sorted().reduce((t, u) -> u).get();//optional dır int e çevirmek için get kullandık
    ...
    public static void minimum2(List<Integer> nums) {
        Integer min = nums.
                stream().
                distinct().
                sorted(Comparator.reverseOrder()).//11 10 5 4 3 2 sayıları ters çeviren method ,sorted ı tek basina kullanırsak küçükten büyüğe sıralar
                reduce((t, u) -> u).get();
